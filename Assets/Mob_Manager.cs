...
public  class Mob_Manager : MonoBehaviour
{
    ...
    public void createMob(Building fromBuildingSpawned, float tileHeight){        
        GameObject onTileSpawned = fromBuildingSpawned.getTile();
        Vector3[] tileCorners = fromBuildingSpawned.GetTileCorners(onTileSpawned);
        //int mobIndex = fromBuildingSpawned.mobCountValue();  
        ...
        Vector3 buildingPosition = fromBuildingSpawned.transform.position;
        float spaceBetweenMobs = Vector3.Distance(topLeft, topRight) / 5; //This is the space between the mobs.


        Vector3 spawnPosition = Vector3.zero;
        float buildingHeight = fromBuildingSpawned.GetComponent<Renderer>().bounds.size.y;
        float mobHeight = mobPrefab.GetComponent<Renderer>().bounds.size.y;
        ...
        GameObject lastAssignedMob = fromBuildingSpawned.GetLastAssignedMob(); //Get the last mob spawned in the building.
        ...
        GameObject mob = Instantiate(mobPrefab, finalSpawnPos, Quaternion.identity);
